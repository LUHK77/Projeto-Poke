...
public class PokeDAO {
    ...
    private Poke criaPokeClass(int id, String apelido, int level, int exp, int hp, int stamina) {
        ...
        switch (id) {
            ...
            case 3 -> {return p = new Piracu(apelido, level, exp, hp, stamina);}
            case 4 -> {return p = new Ashes(apelido, level, exp, hp, stamina);}
        }
        return p = null;
    }
    ...
    public Poke selecionarI(int id) {
        String sql = "SELECT apelido, level_poke,exp_poke,hp,stamina FROM poke WHERE id = ?";
        Poke p = null;
        try (
